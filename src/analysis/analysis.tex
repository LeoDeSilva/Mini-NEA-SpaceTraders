...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item The program should run in the form of a Terminal User Interface (TUI). (\textit{1.4 Existing Systems})
...
\begin{enumerate}
    \item The agent's API access token.
    \item An array of the agent's previously visited waypoints.
    \item The agent's current loan ID \& resource required for completion.
    \item The ID's of all the agent's owned ships.
    \item The number of credits possessed by an agent.
\end{enumerate}
A similar description for the ship class would be as follows:
\begin{enumerate}
    \item The ship's flight mode (CRUISE, BURN, DRIFT, STEALTH).
    \item The ship's waypoint marker.
    \item The ship's fuel level.
    \item The ship's current cargo.
    \item The ship's maximum weight – and the current weight of cargo.
\end{enumerate}
A waypoint struct may store:
\begin{enumerate}
    \item The waypoint's x,y coordinates.
    \item The waypoints symbolic name.
    \item Traits for that particular waypoint e.g. (MARKETPLACE, SHIPYARD).
\end{enumerate}